...
public enum OutputViewMessages implements EnumMessageFormatter {
    ...
    // 구입 금액 입력부분 확인
    OUTPUT_ASK_MENU_NUMBERS("주문하실 메뉴를 메뉴와 겨수를 알려 주세요. "),
    OUTPUT_MENU_ARGUMENT_LIMIT("("+"e.g. %s-%d,%s-%d,%s-%d"+")"),
    // 보너스 번호, 당첨번호 공통 확인 OUTPUT_ERROR
    // print 출력값 포장할 것
    OUTPUT_MONTH_DATE("%d월 %d일에 우테코 식당에서 받을 이벤트 혜택 미리 보기!"),
    OUTPUT_ORDER_LIST_MESSAGE("<주문 메뉴>"),
    OUTPUT_ORDER_LIST_EICH("%s %d개"),
    OUTPUT_RECEIPT_BEFORE_DISCOUNT_MESSAGE("<할인 전 총주문 금액>"),
    OUTPUT_RECEIPT_BEFORE_DISCOUNT_PRICE("%,d%s원"),
    ...
    OUTPUT_COLON(": "),
    OUTPUT_SERVICE_TOTAL_MESSAGE("<총혜택 금액>"),
    OUTPUT_SERVICE_TOTAL_PRICE("%,d원"),
    ...
    OutputViewMessages(String printInterface) {
        this.message = printInterface;
    }
